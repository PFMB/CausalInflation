...
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("screen.randomForest_grid",tuneGrid[i,1], 
...
{
  cat(paste0(" - screen.corRank was started and screens with method = ",method," - "))
  ...
  cat(paste0("- screen.corRank finished lasting: ", round(unclass(screen.corRank_time)["elapsed"],2)," - "))
  return(whichVariable)
}
...
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("screen.corRank_grid",str_replace_all(tuneGrid[i,1], "[[:punct:]]", ""), 
...
screen.cramersv_base <- function(Y, X, nscreen = nscreen, cts.num = 5, ...){
  cat(paste0(" - screen.cramersv was started and screens with nscreen = ", nscreen," - "))
  if(ncol(X)>nscreen){
  screen.cramersv_time <- system.time({
  ...
  cvf <- function(xx){assocstats(table(Y,xx))$cramer}         
  whichVariable <- order(apply(cvdata,2,cvf),decreasing=T)<=nscreen   
  })
  cat(paste0("- screen.cramersv finished lasting: ", round(unclass(screen.cramersv_time)["elapsed"],2)," - "))
  }
  else{
...
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("screen.cramersv_grid",tuneGrid[i,1], 
...
screen.glmnet_base <- function(Y, X, family, alpha = alpha, minscreen = 2, pw=T,
  ...
  if(is.null(whichVariable)){
    ...
    if(pw==T){cat("Lasso failed and only first variable was screened \n")}}
  if(pw==T){cat(paste("Number of included variables:",sum(whichVariable),"\n"))}
  })
  cat(paste0("- screen.glmnet was finished lasting: ", round(unclass(screen.glmnet_time)["elapsed"],2)," - "))
  return(whichVariable)
}
...
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("screen.glmnet_grid",tuneGrid[i,1],"_nlambda",tuneGrid[i,2], 
...
screen.glmnet_nVar <- function(Y, X, family, alpha = 0.75, nfolds = 5, nlambda = 200, nVar = 8,...){
  ...
  cat(paste0("nVar (",nVar,") is not available, instead ", sum(whichVariable)," vars were screened."))
  }
  return(whichVariable)
}
          ...
          minscreen = 2,...) 
{
  ...
  if (sum(whichVariable) < minscreen) {
    cat(paste0("number of variables with p value less than minPvalue is less than minscreen"))
    whichVariable[rank(listp) <= minscreen] <- TRUE
  }
  }else{
  whichVariable <- rep(TRUE,ncol(X))
  }
  cat(paste("CorPearson screens ",sum(whichVariable)," variables."))
  return(whichVariable)
}
...
{
  cat(paste0(" - earth2 was started and is making predictions - "))
  ...
  cat(paste0("- earth2 was finished lasting: ", round(unclass(earth_time)["elapsed"],2)," - "))
  return(out)
}
...
tuneGrid <-  expand.grid(ntree = c(500,1000))
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("SL.randomForest_grid",tuneGrid[i,1], 
                           "<- function(..., ntree = ", tuneGrid[i, 1],")
...
SL.mgcv <- function(Y, X, newX, family, obsWeights , ...) {
  ...
  if(any(class(fit.gam)=="try-error")){
    ...
    if(any(class(fit.gam)=="try-error")){successful_gam <- FALSE
    }else{successful_gam <- TRUE}
...
for(i in seq(nrow(tuneGrid))) { 
  eval(parse(text = paste0("SL.gbm_grid",tuneGrid[i,1],"_interaction.depth",tuneGrid[i, 2],
...
{
  cat(paste0(" - GLM Interaction was started and is making predictions - "))
  ...
  cat(paste0("- GLM Interaction was finished lasting: ", round(unclass(glm_time)["elapsed"],2)," - "))
  return(out)
}
